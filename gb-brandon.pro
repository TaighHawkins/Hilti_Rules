...
i_version( brandon, `05 March 2015` ).

i_date_format( _ ).

i_page_split_rule_list( [ check_for_new_variety ] ).
i_section( check_for_new_variety, [ new_variety_id_line ] ).
i_line_rule_cut( new_variety_id_line, [ 
	check_text( `997320973` )
	, trace( [ `New Variety detected` ] )
	, set( chain, `gb brandon hire` )
] ).
...
%=======================================================================
	...
	[ q10( [ item_code(s), tab ] ), line_item( f( [ begin, q(dec,3, 9), end ]) ) ]

	, [ line_item( f( [ begin, q(dec,3, 9), end ]) ),  tab ]
	
	, [ q10( [ item_code(s), tab ] ), read_ahead([ q0n(word), line_item( f( [ begin, q(dec,3, 9), end ]) ) ]) ]
